...
function formatStatusExtra(msg: StatusMessage): string {
  const bits: string[] = [];
  if (msg.url) bits.push(msg.url);
  if (msg.waitedMs != null) bits.push(`waited ${msg.waitedMs}ms`);
  if (msg.timedOut) bits.push("(timed out)");
  if (msg.readyState) bits.push(msg.readyState);
...
let pendingDoc: WebPageDoc | null = null;

function requestScrape(ws: WebSocket | null, url: string): void {
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    console.log("[app] No extension connected — cannot start scrape.");
    return;
  }
  console.log(`[app] -> asking browser to navigate to: ${url}`);
  const command: NavigateCommand = { action: "navigate", url, ...SCRAPE_OPTS };
...
wss.on("connection", (ws: WebSocket) => {
  ...
  ws.on("message", async (data: RawData) => {
    ...
    try {
      msg = JSON.parse(data.toString()) as FromExtension;
    } catch {
    ...
    switch (msg.type) {
      ...
      case "html": {
        console.log(
          `[app] <- received HTML (${msg.bytes ?? msg.html.length} chars) from ${msg.finalUrl}`
        );
