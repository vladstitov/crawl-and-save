...
export interface OllamaConfig {
  baseUrl: string;
  model: string;
  /** How long to keep the model resident between calls (Ollama `keep_alive`). */
  keepAlive: string;
...
/** A link as stored on a scraped page's `pageLinks`. */
...
/** A reusable prompt: a system instruction plus a per-input user prompt builder. */
export interface Preset<TInput> {
  system: string;
...
export const PRESETS = {
  ...
  secondaryNewsLink: {
    system:
      "You classify hyperlinks found on a private-equity secondaries news " +
    ...
    build: (link: PageLink) =>
      `Is this link a news article about private-equity secondaries?\n` +
      `URL: ${link.url}\n` +
      `Link text: ${link.text?.trim() || "(none)"}\n` +
      `Answer YES or NO.`,
  } satisfies Preset<PageLink>,
} as const;
...
export async function generate(
  prompt: string,
  opts: { system?: string; model?: string; numPredict?: number } = {}
): Promise<string> {
 ...
 */
export async function askPresetYesNo<T>(preset: Preset<T>, input: T): Promise<boolean | null> {
 ...
 *     stall the crawl.
 */
export async function isSecondaryNewsLink(
  link: PageLink,
  ...
  try {
    ...
    console.warn(
      `[ollama] classification failed for ${link.url}: ${err instanceof Error ? err.message : err}` +
